    ...
    if form.validate_on_submit():
        ...
        if form.nomination_start.data >= form.nomination_end.data:
             ...
             election = Election(
                ...
                success_msg=form.success_msg.data,
                status='draft'
             )
...
def edit_election(election_id):
    ...
    if form.validate_on_submit():
        ...
        if form.nomination_start.data >= form.nomination_end.data:
             ...
             election.success_msg = form.success_msg.data
...
def edit_elector(elector_id):
    ...
    if form.validate_on_submit():
        ...
        #BLEACH library used to clean HTML string. To prevent Cross site XSS.
        ALLOWED_TAGS = ['b', 'i', 'u', 'strong', 'em', 'p', 'br', 'ul', 'li', 'table', 'tr', 'td', 'img']
        elector.custom_success_msg = bleach.clean(form.custom_success_msg.data, tags=ALLOWED_TAGS, strip=True)
...
def initiate_release_results(election_id):
    ...
    # Release logic updated for anonymity
...
def verify_release_results_otp():
    ...
    if request.method == 'POST':
        ...
        if is_valid:
            ...
            if election:
                # Anonymized system: No duplicate checks by elector_id possible or needed
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password_hash, password):
                ...
                security_logger.info(f"Admin Login Attempt: {user.username} from {request.remote_addr}")
    ...
    if request.method == 'POST':
        ...
        if is_valid:
             ...
             if user:
                 ...
                 security_logger.info(f"Admin login: {user.username} from {request.remote_addr}")
                 return redirect(url_for('admin.dashboard'))
        else:
             flash(msg, 'error')
...
@login_required
def logout():
    security_logger.info(f"Admin logout: {current_user.username} from {request.remote_addr}")
    logout_user()
    return redirect(url_for('admin.login'))
