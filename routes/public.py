...
def ballot(election_id):
    ...
    if request.method == 'POST':
        # DOUBLE-VOTING GUARD (Back button protection)
        if elector.has_voted:
             session.pop('voter_elector_id', None)
             flash('Security Alert: You have already voted. Your previous vote was recorded.', 'warning')
             return redirect(url_for('public.index'))

        candidate_id = request.form.get('candidate_id')
        if candidate_id:
            # ANONYMIZED VOTE: No elector_id stored
            ...
            return redirect(url_for('public.thanks', election_id=election_id, elector_id=elector.id))
        else:
...
@public_bp.route('/election/<int:election_id>/thanks/<int:elector_id>')
def thanks(election_id, elector_id):
    from models import Elector
    election = Election.query.get_or_404(election_id)
    elector = Elector.query.get_or_404(elector_id)
    
    # Security: Ensure elector belongs to this election
    if elector.election_id != election_id:
        return redirect(url_for('public.index'))
        
    return render_template('public/thanks.html', election=election, elector=elector)
